--[[
    MoveCargo Script - Version: 1.0 - 21/2/2020 by Theodossis Papadopoulos
    -- Requires MIST
    -- With love for =GR= Spanker
    -- DON'T FORGET TO CREATE A SPAWN ZONE
       ]]
...
local function checker()
  for i=1, tablelength(CARGO_SCRIPT) do
    ...
    for j=1, tablelength(all_cargos) do
      ...
      if (isInZone(cargo:getPosition().p, zone)) and (cargo:inAir() == false) and (cargo:getLife() >= 1) then
        cnt = cnt + 1
      end
    end
  ...
  for j=1, tablelength(allUnits) do
    ...
    if us:getGroup():getID() == gpid then -- Found him/them for two seat
      if isInZone(us:getPosition().p, trigger.misc.getZone("SPAWN_ZONE")) then 
        ...
        local crate = {
          ["category"] = "Cargos",
          ["shape_name"] = "ab-212_cargo", -- MIKRO DYXTIOTO
          ["type"] = "uh1h_cargo",
          --[[ Placeholder for different crates 
          
          ["shape_name"] = "bw_container_cargo", -- MEGALO PRASINO
